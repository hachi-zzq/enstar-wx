                            ...
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <select class="form-control" name="book" id="jq-validation-select2">
                                            ...
                                            <option value="{{$book->id}}" @if($book->status == 1) disabled="disabled" @endif>{{$book->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        ...
                                        <p class="help-block text-primary" style="clear: both"><span class="text-danger">（已发布的教材不能添加单元，为不可选状态）</span></p>
                                    </div>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            ...
                            <!-- 10. $FORM_EXAMPLE =============================================================================
                                            ...
                                            Form example
                            -->
                            <form class="panel form-horizontal"  enctype="multipart/form-data" method="post" action="{{route('adminLessonPostCreate')}}">
                                <div class="panel-body">
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-10">
                                            <select class="form-control" name="book_unit" id="jq-validation-select2">
                                                ...
                                                <option value="book_{{$v['id']}}" style="font-weight: bolder"@if($v['id']==$book_id) selected='selected' @endif @if($v['disabled']=='true') disabled='disabled' @endif >{{$v['name']}}(V{{{$v['version']}}})</option>
                                                    @if(!empty($v['units']))
                                                    @foreach($v['units'] as $v_unit)
                                                <option value="book_{{$v['id']}}unit_{{$v_unit['id']}}" style="color: #aaa;padding-left: 10px" @if($v['disabled']=='true') disabled='disabled' @endif>|--{{$v_unit['name']}}</option>
                                                    @endforeach
                                                    @endif
                                                @endforeach
                                            </select>
                                            <p class="help-block text-primary" style="clear: both">选择将要添加的课文的位置，教材或者单元<span class="text-danger">（已发布的教材不能添加课文，为不可选状态）</span></p>
                                        </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-10">
                                            <input type="text" class="form-control" id="inputEmail2" name="lesson_title" placeholder="" value="{{ Session::get('flash_session')['lesson_title'] }}">
                                        </div>
                                    </div> <!-- / .form-group -->
                                    <script>
                                        init.push(function () {
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-10">
                                            ...
                                            <p class="help-block text-primary" style="clear: both">只支持MP3、WAV、M4A格式</p>
                                        </div>
                                    </div> <!-- / .form-group -->
...
<!--                                                <textarea style="width: 600px;height: 300px" name="raw_content" placeholder="原文文本"></textarea>-->
                                                <textarea id=demotext name="raw_content" placeholder="原文文本" style="font-family: consolas"></textarea>
                                            </div>

                                            <div style="float: left;margin-top: 5px">
<!--                                                <textarea style="width:600px;height: 300px" name="asr_content" placeholder="ASR匹配文本"></textarea>-->
                                                <textarea id=demotext_2 name="asr_content" placeholder="ASR匹配文本"></textarea>
                                            </div>
                                            <p class="help-block text-primary" style="clear: both">1.确保两边的每行对应，行数一致</p>
                                            <p class="help-block text-primary" style="clear: both">2.左边原文文本含有特殊标记，"[[A]]"用来标记A说话人、"^"代表此行前面换行、"^^"代表此行换段、"{12}"对应着右边ASR中"{twelve}"</p>
                                            <p class="help-block text-primary" style="clear: both">3.所有行的左边都不能留有空格，必须顶行</p>
                                        </div>
                                    </div> <!-- / .form-group -->
